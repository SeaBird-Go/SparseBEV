...
def render_cuda(
    ...
    for i in range(b):
        ...
        # image, depth, norm, alpha, radii, feats = results
        render_results.append(results)
    
    _render_results = list(zip(*render_results))
    stacked_render_results = [torch.stack(tensors) for tensors in _render_results]

    return stacked_render_results
