...
class VisElement(object):
    def __init__(self, 
                 ...
                 is_sparse=False,
                 direct=False, # for depth map visualization
                 min_value=0.5,
                 max_value=55.0):
...
def visualize_elements(inputs: VisElement,
    ...
    Args:
        ...
        cam_order: the camera order. Defaults to [2,0,1,5,3,4]. original camera order is [
            'CAM_FRONT',
            'CAM_FRONT_RIGHT',
            'CAM_FRONT_LEFT',
            'CAM_BACK',
    ...
    for vis_elem in inputs:
        ...
        if vis_elem.type == 'rgb':  # (num_cam, 3, H, W)
            ...
            if vis_elem.keep_origin:
                visual_imgs = images
            elif vis_elem.need_denormalize:
            ...
            if vis_elem.is_sparse:
                ...
                for b in range(len(depth)):
                    pred_depth_color = visualize_depth(depth[b], direct=vis_elem.direct)
